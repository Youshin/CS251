...
public:
    ...
    void show() const;
    std::string showAlphabets() const;
...
inline int Key::getOffset() const {
	return offset;
}
...
inline std::string Key::showAlphabets() const { 
	std::string str = "";
	std::for_each(m_digit.begin(), m_digit.end(), [&str](const char& c)
	{
		str += ALPHABET[c];
	});
	str += '\0';
